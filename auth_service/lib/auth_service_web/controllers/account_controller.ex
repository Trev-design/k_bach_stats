defmodule AuthServiceWeb.AccountController do
  ...
  def signup(conn, %{"name" => name, "email" => email, "password" => password, "confirm" => confirm}) do
    with {:ok, %Account{id: id, email: _email} = account} <- Accounts.create_account(%{email: email, password: password, password_confirmation: confirm}),
         {:ok, %User{id: _user, name: name}}              <- Users.create_user(account, %{name: name}),
